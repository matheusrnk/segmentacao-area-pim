...
def animate_matrices_with_titles(matrices, titles, interval=1500):
    ...
    ani = animation.FuncAnimation(fig, update, frames=len(matrices), interval=interval, blit=True)
...
def deletes_connected_componentes_on_edges(labeled_matrix: np.ndarray) -> np.ndarray:
    ...
    border_values = np.concatenate([top_row, right_column, bottom_row[::-1], left_column[::-1]], dtype=labeled_matrix.dtype)
...
def find_connected_components(th_img_matrix: np.ndarray, conn_type: str) -> np.ndarray:
    ...
    th_img_matrix_bord = np.pad(th_img_matrix, pad_width=1, mode='constant', constant_values=0)
    ...
    return labels_bord[1:-1, 1:-1].copy() # remove a borda de -1 para retornar só os labels
...
def main(args):
    ...
    labeled_matrix_wo_edges = deletes_connected_componentes_on_edges(labeled_matrix)
    
    # Conta os rótulos e encontra o maior componente (eliminando o fundo)
    biggest_component_label = count_and_find_biggest_in_labeled_matrix(labeled_matrix_wo_edges)
    
    # Retorna matriz da imagem (somente com o componente) gerada
    component_only_matrix = get_connected_component(labeled_matrix_wo_edges, biggest_component_label)
...
if __name__ == '__main__':
    ...
    titles = ['Entrada'] + [f'Transição {i}' for i in range(1, len(matrices)-1)] + ['Saída']
